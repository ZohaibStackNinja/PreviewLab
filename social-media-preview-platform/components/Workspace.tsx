...
export function Workspace({
  ...
  const newProject = useCallback(async () => {
    try {
      const { project: created } = await postJson<{ project: ProjectSummary }>(
        "/api/projects",
        {
      ...
      router.push(
        `/project/${created.id}/${created.lastPlatform || DEFAULT_PLATFORM}`,
      );
    } catch (e) {
      toast(
              ...
              {menuFor === v.id && (
                <div className="variant-menu" role="menu">
                  ...
                  <button
                    ...
                    className="danger"
                    onClick={() => {
                      if (confirm(`Remove “${v.name}”? This cannot be undone.`))
                        removeVariant(v.id);
                      setMenuFor(null);
                    }}
                  >
  ...
  return (
    <div
      className={`app ${presenting ? "presenting" : ""} ${!commentsVisible ? "comments-hidden" : ""}`}
    >
