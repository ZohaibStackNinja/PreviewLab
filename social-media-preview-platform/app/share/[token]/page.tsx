    ...
    <main className="share-state-page">
      <div className="share-state-card">
        <span className={`share-state-icon ${kind}`}>
          {kind === 'expired' ? <ClockIcon size={24} /> : <WarningIcon size={24} />}
        </span>
        <h1>
          {kind === 'expired'
...
export default async function SharePage({ params }: PageProps) {
  const { access, data } = resolveShareToken(params.token);

  if (access === 'EXPIRED') return <UnavailableState kind="expired" />;
  if (access === 'REVOKED' || access === 'INVALID') return <UnavailableState kind="revoked" />;
  if (!data) return <UnavailableState kind="revoked" />;
    ...
    <main className="share-page">
      <header className="share-topbar">
        ...
        <span className="share-expiry-note">
          <ClockIcon size={14} /> Link active · expires {formatDateTime(data.share.expiresAt)} (
      ...
      <div className="share-body">
        <section className="share-stage">
          ...
          <p className="share-stage-sub">
            {data.share.device === 'desktop' ? 'Desktop' : 'Mobile'} · {data.project.title} ·{' '}
            {data.variant.name}
          ...
          <p className="share-footer">
            Simulated preview for review purposes only — not affiliated with YouTube, Instagram,
            Facebook, TikTok or LinkedIn.
          </p>
        </section>

        <aside className="share-comments-aside">
          <ShareComments token={params.token} initialComments={data.comments} />
        </aside>
      </div>
    </main>
  );
